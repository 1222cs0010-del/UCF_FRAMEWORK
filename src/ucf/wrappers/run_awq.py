"""
Wrapper for AWQ baseline (llm-awq).
Provides a smoke-run that uses the template and documents where quantization scripts live.
"""
import argparse
import os
import subprocess
...
def run_smoke(baselines_dir, out_path):
    ...
    subprocess.check_call(["python", tpl, "--baseline", "llm-awq", "--out", out_path, "--workdir", baselines_dir])


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--baselines_dir", default=os.path.abspath(os.path.join(os.getcwd(), os.pardir, os.pardir, "baselines")))
    parser.add_argument("--out", default="results/awq_metrics.json")
