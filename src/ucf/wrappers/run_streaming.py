"""
Wrapper for StreamingLLM baseline.
"""
import argparse
import os
import subprocess
...
def run_smoke(baselines_dir, out_path):
    ...
    subprocess.check_call(["python", tpl, "--baseline", "streaming-llm", "--out", out_path, "--workdir", baselines_dir])


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--baselines_dir", default=os.path.abspath(os.path.join(os.getcwd(), os.pardir, os.pardir, "baselines")))
    parser.add_argument("--out", default="results/streaming_metrics.json")
